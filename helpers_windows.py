...
def downloader(urls):
	...
	browsermobproxy_location = r"{}\browsermob\bin\browsermob-proxy".format(path) 
	
	# Start browsermob server
	
	server = Server(browsermobproxy_location, options=dict)
	server.start()
	time.sleep(1)
	...
	chrome_path = r"{}\chromedriver.exe".format(path)
